...
function get_icomoon_icons()
{
	...
	$subject = file_get_contents(get_template_directory_uri() . '/assets/css/flaticon.css');
	preg_match_all($pattern, $subject, $matches, PREG_SET_ORDER);
	return $matches;
}
...
function immigro_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('immigro_redux_custom_fonts_load', 'immigro_redux_custom_fonts_load');
function immigro_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = immigro_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/immigro/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [immigro_social_share_output description]
 *
 ...
 */
function immigro_social_share_output( $icon, $color = false ) {
...
function immigro_get_social_icons()
{
	...
	if( immigro_set( $options, 'social_media' ) && is_array( immigro_set( $options, 'social_media' ) ) )
	{
		$total = count( immigro_set( $options, 'social_media' ) ) - 2;
		foreach( immigro_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = immigro_set( $social_icon, 'title');
			$class = immigro_set( $social_icon, 'class');
			$link = immigro_set( $social_icon, 'social_link');
			$icon = immigro_set( $social_icon, 'social_icon');
...
add_action('immigro_social_share_output', 'immigro_social_share_output');
...
add_action('immigro_social_share_output_two', 'immigro_social_share_output_two');
...
<ul class="social-links-style1 float-left fix">
	...
	<?php if($options->get( 'facebook_sharing' )):?>
    <li><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><i class="fab fa-facebook"></i></a></li>
    <?php endif;?>
