...
if ( ! function_exists( 'SLIDERPATH_Functions' ) ) {
	class SLIDERPATH_Functions {
...
function get_widgets() {
			...
			$widgets = [
					...
					'sliderpath_slider'  => [
		            'title'      => esc_html__( 'Slider One', 'slider-path' ),
		            'icon'       => 'sliderpath_icons eicon-slides',
		          ...
		          'sliderpath_khulna'  => [
		            'title'      => esc_html__( 'Slider Two', 'slider-path' ),
		            'icon'       => 'sliderpath_icons eicon-slides',
		            'keywords'   => [ 'slider Khulna', 'toggle', 'collapse' ],
		            'styles'     => 2,
						'views'      => [
						'2' => 'template-2',
				
					],
		            'class_name' => 'SLIDERPATH_khulna',
		            'type'       => 'self',
			...
			return apply_filters( 'sliderpath_filters_widgets', $widgets );
		}
		...
		/**
		 * SLIDERPATH_Functions constructor.
		 */
		...
		function is_element_active( $element_id = '' ) {
			...
			return in_array( sprintf( 'sliderpath_%s', $element_id ), $this->get_active_elements() );
		}
		...
		function set_active_elements() {
			$elements_ext_active = $this->get_option( 'sliderpath_elements_ext_active', array_keys( $this->get_widgets_options( 'external' ) ) );
			$elements_ext_active = empty( $elements_ext_active ) || ! is_array( $elements_ext_active ) ? array() : $elements_ext_active;
			$elements_active     = $this->get_option( 'sliderpath_elements_active', array_keys( $this->get_widgets_options( 'self' ) ) );
		...
		function get_template_by_id( $template_id = '', $template_group = '' ) {
			...
			return apply_filters( 'sliderpath_filters_template_by_id', $template, $template_id, $template_group );
		}
		...
		function get_template_group_thumb( $template_group = array() ) {
			...
			$pages    = sliderpath()->get_settings_atts( 'pages', array(), $template_group );
			$template = reset( $pages );
			$thumb    = sliderpath()->get_settings_atts( 'thumb', '', $template );

			return apply_filters( 'sliderpath_filters_get_template_group_thumb', $thumb );
		}
...
//this code will be delted		
		function get_plugin_data( $data_for = 'templates' ) {

			// Check option for saved data
			$sliderpath_api_response = sliderpath()->get_option( 'sliderpath_api_response', array() );
			$sliderpath_api_response = ! is_array( $sliderpath_api_response ) ? array() : $sliderpath_api_response;

			// If empty get from remote server
			if ( empty( $sliderpath_api_response ) ) {
				$sliderpath_api_response = sliderpath()->get_plugin_data_from_api();

				update_option( 'sliderpath_api_response', $sliderpath_api_response );
			}

			return sliderpath()->get_settings_atts( $data_for, array(), $sliderpath_api_response );
		}
		...
		function get_plugin_data_from_api() {
			if ( is_wp_error( $api_response = wp_remote_get( sprintf( '%s/wp-json/sliderpath/plugin-data', SLIDERPATH_API_URL ) ) ) ) {
				return array();
			}
		...
		function include_widget_class( $widget_slug = '' ) {
			...
			$widget_class_file = sprintf( '%swidgets/%s/widget.php', SLIDERPATH_PLUGIN_DIR, $widget_slug );
			$widget_class_file = apply_filters( 'sliderpath_filters_widget_class_file', $widget_class_file, $widget_slug );
		...
		function get_widgets_options( $for = '' ) {
			...
			return apply_filters( 'sliderpath_filters_widgets_options', $widgets, $for );
		}
		...
		function get_post_ids( $content_type = '' ) {
			...
			if ( $content_type == 'by_posts_category' ) {
				...
				foreach ( sliderpath()->get_settings_atts( '_category' ) as $category_id ) {
				...
				echo '<pre>';
				print_r( sliderpath()->get_settings_atts( 'tag_ids' ) );
				echo '</pre>';
				...
				$post_ids = get_posts( array(
					...
					'tax_query'      => array(
						array(
							'taxonomy' => 'post_tag',
							'field'    => 'term_id',
							'terms'    => sliderpath()->get_settings_atts( 'tag_ids' ),
						),
					)
				) );
				...
				$post_ids = explode( ',', sliderpath()->get_settings_atts( 'custom_post_ids' ) );
			} else {
				$post_ids = sliderpath()->get_settings_atts( 'post_ids' );
			}
		...
		function get_advanced_addons() {
			return apply_filters( 'sliderpath_filters_advanced_addons', array(
		...
		function get_option( $option_key = '', $default_val = '' ) {
			...
			return apply_filters( 'sliderpath_filters_option_' . $option_key, $option_val );
		}
		...
		function load_widget_styles( $style_count = 1 ) {
			...
			for ( $index = 1; $index <= $style_count; $index ++ ) {
				$styles[ $index ] = sprintf( esc_html__( 'Style %s', 'slider-path' ), $index );
			}
