...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => immigro_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => immigro_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => immigro_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => immigro_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => immigro_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => immigro_meta( 'banner_elementor_template', $page_id ),
				'banner'            => immigro_set( immigro_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => immigro_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => immigro_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : immigro_the_title( $template ),
				'banner'        => immigro_set( $bg, 'url' ),
