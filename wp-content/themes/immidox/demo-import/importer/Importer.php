...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( __( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'immigro' ) );
		...
		if ( ! $status ) {
			$this->logger->error( __( 'Could not open the XML file for parsing!', 'immigro' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( __( 'Content import start error: ', 'immigro' ) . $result->get_error_message() );
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( __( 'New AJAX call!', 'immigro' ) );
	...
	public function woocommerce_product_attributes_registration( $data ) {
		...
		if ( strstr( $data['taxonomy'], 'pa_' ) ) {
			if ( ! taxonomy_exists( $data['taxonomy'] ) ) {
				...
				if(function_exists('immigro_taxonomy_regster') ) {
					
					// Register the taxonomy now so that the import works!
					immigro_taxonomy_regster(
						$data['taxonomy'],
